...
- **Isolate a best-effort side-effect from the operation it follows — separate try/catch.** A best-effort side-effect (e.g. `localStorage` persistence) sharing ONE try/catch with the operation's success commit lets its failure masquerade as an operation failure: the action actually succeeded, yet a misleading error banner appears. Rule: surface the operation's OWN failure via `messageFromReason` + an early `return` (see "normalize every rejection" above), then run each best-effort side-effect in a SEPARATE try/catch that only `console.error`s. A best-effort failure must never reach the user-facing `error` state. Canonical: `jobStore.setOutputDir` commits the returned draft (or returns on real failure), then persists in its own try/catch.
...
- **Validate before using persisted/deserialized external values.** Do not blind-cast `localStorage.getItem(key) as T`; use a type guard and fall back to a safe default on an unrecognized value. (`isTheme` in `theme-provider.tsx` is the canonical shape; `isValidOutputDir` in `src/lib/output-dir-default.ts` follows the same pattern.) Two extensions:
  - **Browser/WebView storage can throw — wrap every read AND write in try/catch.** `localStorage.getItem`/`setItem` (even *accessing* `localStorage`) can throw: a `SecurityError` when storage is disabled or restricted (private mode, a locked-down WebView), a `QuotaExceededError` when full. Treat any such failure as non-fatal — `console.error` + fall back to the default. (Canonical: `loadPersistedOutputDir` / `persistOutputDir`.)
  - **Guard the WRITE boundary too — never persist an invalid value.** Run the same type guard before writing so a corrupt value can never be stored (`persistOutputDir` early-returns when `!isValidOutputDir(dir)`), not only before reading.
...
- **Confirmation/modal dialogs: native `<dialog>`, not `<div role="dialog">` (a11y harness).** `jsx-a11y/prefer-tag-over-role` (the same rule that mandates `<output>` over `<div role="status">` above) treats `role="dialog"` as a **hard error** in `pnpm lint:ci`. Use the native `<dialog>` element: toggle visibility with the `open` attribute or `showModal()`, label it with `aria-labelledby` / `aria-describedby`, and dismiss on Escape and backdrop click to `cancel`. **Do not add a dialog library** (shadcn `Dialog` / Radix Dialog) — the native element is sufficient. Canonical shape: `src/components/ui/confirm-dialog.tsx`.
