...
- **A React index `key={i}` is correct for a positional, backend-ordered list** with no per-row local state whose item values may legitimately duplicate (`TaskList` rows). Suppress Biome's `lint/suspicious/noArrayIndexKey` with a targeted `// biome-ignore` that carries the rationale, rather than synthesizing a fake unique key.
