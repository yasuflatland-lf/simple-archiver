...
`simple-archiver` is a **Mac/Windows native desktop app** that lets you drag and drop multiple rar/zip files and folders and produce either renamed zip archives or extracted folders.

- `OutputMode::Zip` extracts rar/zip inputs before re-compressing them and zips folder inputs as-is; `OutputMode::Folder` extracts archive inputs and places their trees as folders
- processed by N parallel workers; sequence numbers start at the draft's configured number and are fixed at job-creation time in list order (independent of completion order)
- output goes to a single user-specified folder; `ConflictPolicy` selects auto-rename, skip, or overwrite at a collision, and defaults to `AutoRename` (see the authoritative definitions in [the domain model](../.claude/domain-model.md))
...
WHY: `OutputMode` splits planning and execution at the top level, while `ConflictPolicy` determines the destination-side effect for either branch.
...
presentation   src-tauri/src/presentation/ — Tauri commands, session state, DTO/event wiring
               commands.rs  — 12 Tauri commands: nine draft mutations (add_items, reorder, remove_item, set_naming_rule, set_start_number, set_output_dir, set_output_mode, set_conflict_policy, clear_items), preview_output_name, run_job, cancel_job + run_job_inner testability seam
...
application    simple-archiver-core — use case / orchestration
               RunArchiveJob: N parallel workers, progress aggregation, cancellation, error tally / ports: OutputStrategy, Extractor, Archiver, Placer, Clock / FormatRegistry: routes SourceItem to prepared dir
domain         simple-archiver-core — pure, no IO (the main TDD battleground)
               ArchiveJob (aggregate root) / ArchiveTask / NamingRule / SequenceNumber / SourceItem / OutputDirectory / OutputMode / ConflictPolicy
infrastructure simple-archiver-core — isolates the variation / adapters
               ZipOutput / FolderOutput / ZipArchiver(async_zip) / ArchiveExtractor + UnrarExtractor + ZipExtractor / FsPlacer / TempWorkspace / SystemClock
```
...
- **`ArchiveJob` is the aggregate root** (PR4): it holds an ordered `Vec<ArchiveTask>` as the single source of truth. The sequence number and resolved output name are *re-derived from list position* on every read — never stored as a second copy. WHY: the aggregate encapsulates the "ordering ↔ sequence ↔ output name" invariant so the PR5 execution engine can trust it without extra synchronisation. Output names are unique within the job (compared case-insensitively via ASCII case-fold; non-ASCII Unicode folding is deferred). `TaskId` is a stable identity separate from position; it never doubles as a sequence number.
- **IO is isolated behind five ports** (`OutputStrategy` prepared tree→output / `Extractor` archive→temporary tree / `Archiver` tree→zip / `Placer` tree→output folder / `Clock`), with implementations in `infrastructure`. PR-5a evolved `Archiver` to `trait Archiver: Send + Sync` with `fn compress(&self, src, dest, ctx: &CompressContext) -> impl Future<Output = Result<(), ArchiveError>> + Send` (RPITIT). WHY: the engine drives implementations across `tokio::spawn`, which needs a `Send` future and a `Send + Sync` trait. `Clock` (`fn now(&self) -> Instant`, `Send + Sync`) was added so `elapsed` is testable with a `FixedClock`. `Extractor` is `Send + Sync` and uses the RPITIT form matching `Archiver`: `fn extract(&self, src_rar: &Path) -> impl Future<Output = Result<Box<dyn ExtractedTree>, ExtractError>> + Send` (no `#[allow(async_fn_in_trait)]`), since the engine drives it across `tokio::spawn` just like the archiver. `Placer::place` returns the actual destination or `PlaceError`; `FsPlacer` performs the blocking recursive copy on `spawn_blocking` and applies the job's collision policy. WHY: Folder-mode writes remain replaceable and testable without filesystem IO entering the application layer. `OutputStrategy::produce` sits in front of `Archiver`/`Placer` so the engine handles every output mode through one port (`ZipOutput` wraps `ZipArchiver`, `FolderOutput` wraps `FsPlacer`); it answers `Produced::At` / `KeptExisting` / `Nothing`, where `Nothing` covers a task whose `OutputName` yields no destination.
...
- **IPC error boundary**: domain errors (`NamingRuleError` / `NameError`) cross to the frontend as a `String` via `e.to_string()` at the Tauri command layer. The exact message text is a shared contract pinned by tests on both sides (Rust command test and frontend test assert the same string). WHY: the Tauri command is a thin bridge with Rust as the single source of truth — no validation logic is duplicated in TypeScript. The classification error `UnsupportedSourceItem` likewise crosses as the byte-stable string `"unsupported item: <path>"`, contract-pinned by tests (domain `unsupported_source_item_display_matches_ipc_contract` + presentation `classify_path_other_file_is_err`). The only naming-related outward touch points are the `preview_output_name` Tauri command (presentation) and the React `NamingRuleForm`.
...
- **`ZipArchiver` forces durability before returning.** `tokio::fs::File` writes on the blocking pool and its `Drop` does **not** wait for them, so a naive return can hand back an unflushed/locked archive — an order/timing-dependent CI flake. After `async_zip`'s `close()` writes the central directory, recover the file via `.into_inner()`, then `shutdown().await` + `sync_all().await` so the zip is complete and readable immediately. Before creating the file, `resolve_destination` applies `ConflictPolicy`: auto-rename chooses a free sibling, skip returns a successful no-op, and overwrite removes the existing file. `OpenOptions::create_new(true)` remains the concurrent-writer race guard after that resolution.
...
- **Mode/policy dispatch:** `RunArchiveJob` snapshots each task's domain-computed destination and hands it to the injected `OutputStrategy::produce` together with the job's `ConflictPolicy` — the engine itself no longer branches on `OutputMode`. The mode is chosen once at `commands.rs::run_job_inner`, which monomorphises the engine over `ZipOutput` (prepared tree → `Archiver::compress`) or `FolderOutput` (extracted tree → `Placer::place`). WHY: one planned job carries the collision choice through every worker without presentation-side recomputation, and a new output mode adds a strategy instead of an engine branch; see [the domain model](../.claude/domain-model.md) for the authoritative invariants.
...
**Zip input support (feature/zip_dandd, implemented):** `.zip` files are now accepted as inputs alongside `.rar` and `Folder`. The pipeline is unchanged — any archive input is extract → rename → recompress to a standard Deflate zip; output is always a normalized zip regardless of input format.

- **`ArchiveExtractor` (router) behind the `Extractor` port.** Production DI wires `ArchiveExtractor` (not `UnrarExtractor`) at `commands.rs::run_job_inner`. `ArchiveExtractor { rar: UnrarExtractor, zip: ZipExtractor }` dispatches by file extension (ASCII-case-insensitive): `.rar` → `UnrarExtractor`, `.zip` → `ZipExtractor`, else → `ExtractError::Backend("unsupported archive: …")` as a defensive fallback (`classify` already gates inputs at the IPC boundary, so this path is hit only if a caller bypasses classification). The generic engine `RunArchiveJob<O, E>` and `FormatRegistry<E>` do not change when an input format is added — only the injected `E` changes. This is the pattern for adding archive formats without touching the application layer.
...
**Adding a new input format (harness — extension checklist).** Because an enum-variant addition cascades, adding a format (e.g. 7z) requires, in lockstep:

1. `SourceItem::<Fmt>File` variant + `classify` arm in `domain` — produces compile errors at every exhaustive `SourceItem` match (good).
2. A new `<Fmt>Extractor` implementing the `Extractor` port in `infrastructure`, plus a router arm in `ArchiveExtractor`. **NOTE:** the router's extension strings are NOT compiler-checked against `classify` — they can silently diverge. A `TODO` comment in `archive_extractor.rs` marks this. Both places must be updated by hand.
3. `FormatRegistry::prepare` match + `run_archive_job` `needs_extract` (`matches!` is non-exhaustive — easy to miss; no compile error on the added variant).
...
The `SourceItem` variant addition drives compile errors at the exhaustive matches (safe), but `matches!` macro sites and the router extension strings fail silently — treat steps 3 and 2 as the highest-risk omissions.

**Adding a new output mode (harness — extension checklist).**

1. Add the domain `OutputMode` variant and its output-name/destination planning rules.
2. Add one infrastructure strategy implementing `OutputStrategy`, composed from focused adapters as needed.
3. Add one monomorphised strategy-selection arm in `commands.rs::run_job_inner`; `RunArchiveJob` stays unchanged.
4. Mirror the mode at the presentation/IPC and frontend boundaries, then regenerate the ts-rs bindings.
5. Add strategy contract tests and run the existing engine and end-to-end suites unchanged.
...
`JobDraft` (in `state.rs`) accumulates the user's pending job configuration inside `AppState`, which is registered with `tauri::Builder::manage`. Nine mutation commands (`add_items` / `reorder` / `remove_item` / `set_naming_rule` / `set_start_number` / `set_output_dir` / `set_output_mode` / `set_conflict_policy` / `clear_items`) each mutate the draft and return a `DraftSnapshot`; with `preview_output_name`, `run_job`, and `cancel_job`, the real `invoke_handler!` registration in `src-tauri/src/lib.rs` contains 12 commands. (`clear_items` removes all queued items while preserving the naming template, start number, and output settings.) `run_job` calls `JobDraft::build`, which dispatches through the mode-specific domain planning path, then hands the planned job to the engine. WHY: the complete command surface keeps Rust as the single source of truth, including every output-setting mutation.

`JobDraft::start_number` defaults to `1`; `JobDraft::set_start_number` accepts any `u32`, and the snapshot returns it to the frontend. When `jobStore.clearResults` clears a completed batch, it advances the value by the result count with `nextStartAfterBatch` (clamped to `u32::MAX`), calls `set_start_number`, then clears the items. WHY: the next Zip batch continues numbering without the frontend diverging from backend-held session state.
...
The whole IPC surface lives in **one** module of thin `invoke` / `listen` wrappers — `addItems` / `reorder` / `removeItem` / `setNamingRule` / `setStartNumber` / `setOutputDir` / `setOutputMode` / `setConflictPolicy` / `clearItems` / `previewOutputName` / `runJob` / `cancelJob` / `subscribeProgress`, plus the `PROGRESS_EVENT = "archive://progress"` constant (mirroring `dto.rs`). New Tauri commands/events are added here, not scattered across components. (An earlier plan proposed separate `commands.ts` / `events.ts`; the implemented reality is the single `archive.ts`.)

### Store is a thin mirror of the backend (`src/store/jobStore.ts`)

A zustand store holds the UI state. Each mutating action calls the corresponding command and **replaces `draft` with the returned `DraftSnapshot`** — there are **no optimistic updates and no client-side recomputation** of output names or sequence numbers. Output names come from the backend `preview_output_name`; each preview sequence is `draft.startNumber + i`. Progress is wired once on mount via `subscribeProgress(applyProgress)` and unlistened on unmount (the async unlisten is guarded against unmount-before-resolve).
...
The OUTPUT group (`OutputSettings`) reads top-to-bottom as: the **hero full landing path** (biggest, monospace; only rendered once the preview filename resolves, so the destination directory is never shown in isolation), then **aligned Destination/Name rows** that share one label column and one control column. `OutputDirPicker` shows the chosen path or a `(not set)` empty state with a `Required` badge. The **readiness chip** now lives in `RunControls` (rendered immediately left of the Run button, idle-only) — it was moved off `OutputSettings` so the visual readiness cue sits next to the action it gates and shares the same `Readiness` value (`src/lib/readiness.ts`) that drives Run's `aria-disabled` reason; `OutputSettings` is purely the editing surface. The cross-row alignment is a CSS-only contract (see the layout note in conventions.md).

A **smart default output directory** is applied on mount by `App.tsx`: `src/lib/output-dir-default.ts` resolves the persisted last choice from `localStorage` (key `simple-archiver-output-dir`, validated via the `isValidOutputDir` type guard) and falls back to the OS Downloads folder via the Tauri path API (`downloadDir()`); both reads are non-throwing. Persistence is **best-effort** — `jobStore.setOutputDir` commits the backend result first, then persists in a separate try/catch (see conventions.md "best-effort side-effect isolation"). This is presentation/lib-layer TypeScript using the Tauri path API + `localStorage`; the Rust backend and the single-source-of-truth draft are unchanged — the backend still owns job state, the smart default only seeds the picker.
