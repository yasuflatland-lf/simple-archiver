...
application    simple-archiver-core — use case / orchestration
               RunArchiveJob: N parallel workers, progress aggregation, cancellation, error tally / ports: Extractor, Archiver, Placer, Clock / FormatRegistry: routes SourceItem to prepared dir
...
infrastructure simple-archiver-core — isolates the variation / adapters
               ZipArchiver(async_zip) / ArchiveExtractor + UnrarExtractor + ZipExtractor / FsPlacer / TempWorkspace / SystemClock
```
...
- **Mode/policy dispatch:** `RunArchiveJob` snapshots each task's domain-computed destination, then `OutputMode::Zip` sends the prepared tree to `Archiver::compress` while `OutputMode::Folder` sends an extracted archive tree to `Placer::place`; both receive the job's `ConflictPolicy`. WHY: one planned job carries the branch and collision choice through every worker without presentation-side recomputation; see [the domain model](../.claude/domain-model.md) for the authoritative invariants.
