...
- **Presentation (Tauri commands)**: a `#[tauri::command]` async fn stays directly callable from a `src-tauri/tests/*.rs` integration test (the macro does not consume the original fn), so the command seam (argument names `src`/`out`, the `ArchiveError`→`String` IPC mapping) is verified by automated tests rather than left unchecked — the core smoke test calls `ZipArchiver` directly and the frontend test mocks `invoke`. Requires: the `presentation` module is `pub`, the lib crate is `simple_archiver_lib` with `[lib] crate-type` including `"rlib"`, and dev-deps `tokio` (macros, rt-multi-thread) + `zip` + `tempfile`.
