...
- **Infrastructure**: narrow integration tests against small real fixtures (small folder, small .rar). Few and slow, so isolate them with a marker. **RAR fixture creation (harness — PR8):** RAR is a proprietary format — there is no open-source tool that *creates* a `.rar` archive (7-Zip/`7zz` can list and extract rar but cannot author one; only WinRAR/`rar` can, and it is absent from CI/dev). To commit a `.rar` test fixture without the proprietary tool, hand-craft a minimal RAR4 "stored" (uncompressed, METHOD 0x30) archive in a script: write the marker block `52 61 72 21 1A 07 00`, a MAIN header (type 0x73), then a FILE header (type 0x74) whose HEAD_CRC is the low 16 bits of `crc32` over the header bytes and whose FILE_CRC32 is `crc32` of the raw file data (Python `zlib.crc32`), followed by the stored bytes; verify with `7zz l` / `7zz x`. The committed fixture `crates/core/tests/fixtures/sample.rar` is exactly this (single `hello.txt` = `hello rar\n`, 71 bytes total). The final correctness oracle is the Rust `unrar`-backed integration test, not just `7zz`.
